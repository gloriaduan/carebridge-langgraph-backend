...
def api_search(package_id: str, filters: dict) -> dict:
    ...
    filters_clean = {key: value for key, value in filters_dict.items() if value != "" and value is not None}
    ...
    if package["result"]["resources"]:
        ...
        if resource["datastore_active"]:
            ...
            if len(filters_clean) > 0:
                p = { "id": resource["id"], "limit": 50, "filters": json.dumps(filters_clean)}
            else:
...
def geocode_address(address: str) -> dict:
    ...
    try:
        ...
        geocode_result = gmaps.geocode(address, region='ca',
        ...
        if geocode_result:
            ...
            # Store in cache
            c.execute('INSERT OR REPLACE INTO geocode_cache (address, lat, lng) VALUES (?, ?, ?)', (address, lat, lng))
            conn.commit()
...
def prune_results(results: List[Dict[str, Any]], essential_keys: List[str]) -> List[Dict[str, Any]]:
    """
    ...
    for item in results:
        ...
        for key in essential_keys:
            if key in item and item[key] is not None:  # Keep if key exists and value is not None
